...
class BasicWrapper:
    ...
    def get_dataset(self, split) -> datasets.Dataset:
        assert self.status == 'well-split', f"Dataset is not well split into train,dev,test in {self.data_dir}"
        assert split in ['train', 'valid', 'test']
        data = self._load_json_and_formatted(os.path.join(self.data_dir, split+'.json'))
        dataset = datasets.Dataset.from_dict(data)
        return dataset
